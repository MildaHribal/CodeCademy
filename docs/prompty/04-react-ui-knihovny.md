# Zadání: sekce `react-ui-knihovny` — UI knihovny a animace v Reactu
...
Sekce **`react-ui-knihovny`** je napsaná, ale **jeden workshop má rozbité testy** a chybí pojmy.

### Stav na disku

Prošlo: `workshop-sada-komponent` (18 kroků), `shadcn-a-radix`.
Neprošlo: `tailwind-v-reactu` (pojmy), `workshop-pristupne-komponenty` (19 kroků, 8 kroků
s chybami).

### Co je potřeba udělat

1. **Oprav `workshop-pristupne-komponenty`.** Kontrola hlásí, že **řešení autora neprojde
   vlastními testy** v krocích 002, 006, 007, 008, 011, 012 a 016. Konkrétní hlášky:
   - krok 002: „Vidět má být právě jeden panel", „Obsah zavřené záložky se nemá vykreslovat",
     „Profil se už vykreslovat nemá"
   - krok 006: „Druhá položka vybraná klávesnicí má zařízení odhlásit"
   - krok 007: „Zvýrazněná nebezpečná položka má mít červené pozadí"
   - krok 008: „Fokus se má vrátit na tlačítko, kterým se dialog otevřel"
   - krok 011: „Po výběru má být ve spouštěči English"
   - krok 012: „Zapnutý přepínač má puntík posunutý doprava"
   - krok 016: „Pod seznamem má být tlačítko Odhlásit všechna zařízení" a dvakrát
     `TypeError: helpers.click: čekal jsem prvek stránky, dostal jsem null`

   U každého kroku zjisti, co je špatně (většinou test hledá prvek, který řešení nevytváří,
   nebo hledá špatně), a **oprav tak, aby test i řešení dávaly smysl**. Nezjednodušuj test jen
   proto, aby prošel — musí dál ověřovat to, co krok učí.

2. **Radix komponenty se vykreslují mimo strom komponenty** (portál). V testu je proto hledej
   nad `document`, ne uvnitř kontejneru:

```js
await helpers.click(document.querySelector('[data-testid="otevrit-dialog"]'));
await helpers.flush();
const dialog = document.querySelector('[role="dialog"]');
assert.ok(dialog, 'po kliknutí se má otevřít dialog');
```

3. **Doplň pojmy** hlášené jako `[S5]`: `slučování tříd`, `varianta komponenty`,
   `stavový atribut` a další, které kontrola vypíše.

4. **Oprav varování `[W3]`** v krocích 010, 011 a 012: popis kroku obsahuje řádek řešení
   (`data-testid="dialog-smazani"`, `aria-label="Jazyk aplikace"`, `id="explicitni"`).
   Ve druhé třetině workshopu se řešení neopisuje — popiš cíl a nech studenta atribut vymyslet,
   nebo ho zmiň slovně bez celého řádku.

5. **Chybí `cards.md`** — 20–30 kartiček (cva varianty, `tailwind-merge`, přístupné vzory,
   `AnimatePresence`, kdy animovat layout).

### Technické poznámky

- Runtime `react`. Tailwind třídy fungují, ale **barvy jsou v `oklch`** — když test čte
  `getComputedStyle`, porovnávej s tolerancí nebo přes `color-mix` výsledek, ne na přesný řetězec.
- Animace přes `motion/react` testuj přes koncový stav a `helpers.waitFor`, ne přes čas.
