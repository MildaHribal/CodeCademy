...
/** Další motiv při kliknutí na přepínač: systém → světlý → tmavý → systém. */
export function nextTheme(theme) {
  const index = THEMES.indexOf(theme);
  return THEMES[(index + 1) % THEMES.length];
}

/** Motiv, který se opravdu použije: u „system" rozhodne preference systému. */
...
/** Popisky přepínače šířky náhledu; `title` vysvětlí, co volba znamená. */
...
/** previewWidth z nastavení → viewport pro Preview.setViewport (null = celý panel). */
