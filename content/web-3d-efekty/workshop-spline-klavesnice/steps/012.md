---
title: Seřaď natočení při scrollu
kind: parsons
see: web-3d-efekty/spline#scroll-a-mys
---

# --description--

Když návštěvník projíždí sekcí, klávesnice se má pomalu natáčet: při vjezdu sekce zespodu je natočená doleva, uprostřed čelem a při odjezdu nahoru doprava. Natočení řídí stránka, ne Spline, takže ho později půjde vypnout.

Průběh sekce spočítáš vzorcem z lekce: 0 ve chvíli, kdy horní okraj sekce dosáhne spodku okna, 1, když spodní okraj odjede nad okno. Průběh omezený na 0 až 1 posuneš o polovinu (střed je 0) a vynásobíš největším natočením.

Seřaď řádky, které přidáš na konec `loadScene`, a doplň jmenovatel průběhu. Dva řádky jsou navíc.

# --hints--

Když je sekce přesně uprostřed okna, klávesnice je natočená čelem (rotace y ≈ 0).
...
window.scrollTo(0, top + stage.offsetHeight / 2 - window.innerHeight / 2);
await helpers.waitFor(() => Math.abs(keyboard.rotation.y) < 0.03).catch(() => {});
assert.ok(Math.abs(keyboard.rotation.y) < 0.03, `se sekcí uprostřed okna má být rotace y ≈ 0, je ${keyboard.rotation.y.toFixed(3)}`);
```

Když je horní okraj sekce u horního okraje okna, rotace odpovídá průběhu sekce.
...
const expected = (window.innerHeight / (window.innerHeight + stage.offsetHeight) - 0.5) * 1.2;
await helpers.waitFor(() => Math.abs(keyboard.rotation.y - expected) < 0.03).catch(() => {});
assert.ok(Math.abs(keyboard.rotation.y - expected) < 0.03, `s horním okrajem sekce u horního okraje okna má být rotace ${expected.toFixed(3)}, je ${keyboard.rotation.y.toFixed(3)}`);
```

Když je sekce hluboko pod oknem, rotace zůstane na dolní hranici −0.6.
...
assert.ok(Math.abs(keyboard.rotation.y + 0.6) < 0.01, `se sekcí pod oknem má být rotace omezená na -0.6, je ${keyboard.rotation.y.toFixed(3)}`);
```
...
```js
  ...
  window.addEventListener('scroll', () => {
    ...
    const progress = (window.innerHeight - rect.top) / (__1__);
...
keyboard.rotation.y += progress * 1.2;
```

## --blanks--

```text
1: window.innerHeight + rect.height
1: rect.height + window.innerHeight
```
