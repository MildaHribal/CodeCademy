---
title: Obrázek zmizí, až je scéna hotová
see: web-3d-efekty/spline#vykon-velikost-sceny-line-nacitani-a-nahradni-obrazek
---

# --description--

Náhradní obrázek má být vidět, dokud se scéna načítá, a zmizet až ve chvíli, kdy je hotová. CSS už je připravené: když má `.explore__stage` třídu `is-loaded`, obrázek plynule zprůhlední.

`app.load` vrací Promise. Na její splnění počkáš přes `await`, jenže `await` jde použít jen uvnitř `async` funkce (nebo na nejvyšší úrovni modulu). Kód proto zabalíš do funkce, kterou hned zavoláš — v dalších krocích do ní přibude práce se scénou.

Na jiných datech, video, které se ukáže, až se dá přehrát:
...
**Úkol:** Aplikaci nech vzniknout jako dosud. Načtení scény přesuň do `async function loadScene()`, která počká na `app.load(…)` a teprve pak přidá `stage` třídu `is-loaded`. Funkci zavolej.

# --hints--

Po načtení scény má `.explore__stage` třídu `is-loaded`.
...
await helpers.waitFor(() => stage.classList.contains('is-loaded')).catch(() => {});
assert.ok(stage.classList.contains('is-loaded'), 'po načtení scény má .explore__stage dostat třídu is-loaded');
assert.ok(Application.course.instances[0]?.findObjectByName('Klávesnice'), 'scéna se má dál načítat');
```

Náhradní obrázek po načtení zprůhlední.

```js
await helpers.importFile('main.js');
const poster = document.querySelector('.explore__poster');
await helpers.waitFor(() => Number(getComputedStyle(poster).opacity) < 0.05).catch(() => {});
assert.ok(Number(getComputedStyle(poster).opacity) < 0.05, 'po načtení scény má být náhradní obrázek průhledný');
```

`main.js` na načtení čeká přes `await` v `async` funkci `loadScene`.

```js
const source = helpers.stripComments(files['main.js'], 'js');
assert.match(source, /async\s+function\s+loadScene\s*\(|loadScene\s*=\s*async\b/, 'v main.js má být async funkce loadScene');
assert.match(source, /await\s+app\.load\s*\(/, 'loadScene má čekat na načtení: await app.load(…)');
```
