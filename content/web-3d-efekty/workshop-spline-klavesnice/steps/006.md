---
title: Když se scéna nenačte
see: web-3d-efekty/spline#typicke-chyby-a-pasti
---

# --description--

Veletrh ukázal, že se načtení scény může pokazit: výpadek sítě, blokovaný soubor, starý prohlížeč. `await app.load(…)` pak vyhodí chybu, `loadScene` skončí a v konzoli zůstane `Uncaught (in promise)`. Návštěvník nic nepozná, jen obrázek zůstane — což je vlastně dobře, jen by měl vědět proč.

Chybu z `await` zachytíš stejně jako u `fetch`. Kandidáti na reakci: stavový řádek `.explore__status` (proměnná `status` je připravená, má `role="status"`, takže ho přečte i čtečka), uvolnění nepoužité aplikace přes `app.dispose()` a konec funkce, aby se nepokračovalo se scénou, která neexistuje.

**Úkol:** Když `app.load` selže, napiš do `status` krátkou českou zprávu, že 3D model se nepodařilo načíst, uvolni aplikaci a z `loadScene` odejdi. Náhradní obrázek zůstane vidět. Když se načtení povede, chová se stránka jako dosud.

# --hints--

Když načtení selže, stavový řádek ukáže zprávu a náhradní obrázek zůstane vidět.

```js
const { Application } = await helpers.importFile('spline-runtime.js');
await helpers.importFile('main.js');
Application.course.failNextLoad = true;
...
assert.ok(status.textContent.trim().length > 0, 'po selhání načtení má .explore__status obsahovat zprávu pro návštěvníka');
assert.ok(!stage.classList.contains('is-loaded'), 'po selhání načtení nesmí .explore__stage dostat is-loaded — obrázek má zůstat');
assert.ok(Number(getComputedStyle(document.querySelector('.explore__poster')).opacity) > 0.95, 'po selhání načtení má být náhradní obrázek vidět');
```

Po selhání se aplikace uvolní a v konzoli nezůstane nezachycená chyba.
...
Application.course.failNextLoad = true;
const rejections = [];
window.addEventListener('unhandledrejection', (event) => rejections.push(event.reason?.message ?? String(event.reason)));
document.querySelector('.explore__stage').scrollIntoView();
await helpers.waitFor(() => Application.course.log.some((entry) => entry.method === 'dispose'), 3000).catch(() => {});
await helpers.wait(100);
assert.deepEqual(rejections, [], 'chyba z app.load má být zachycená, ne Uncaught (in promise)');
assert.ok(Application.course.log.some((entry) => entry.method === 'dispose'), 'po selhání načtení zavolej app.dispose()');
```

Když se načtení povede, obrázek zmizí a stavový řádek je prázdný.
...
assert.ok(stage.classList.contains('is-loaded'), 'po úspěšném načtení má .explore__stage dostat is-loaded');
assert.equal(document.querySelector('.explore__status').textContent.trim(), '', 'po úspěšném načtení má být .explore__status prázdný');
```
...
Zachycení chyby z `await` znáš z `js-async`. Proč obrázek nechat a tlačítka neblokovat chybou, je v pastech v části [Typické chyby a pasti](see:web-3d-efekty/spline#typicke-chyby-a-pasti).

## --tip--

Obal `await app.load(…)` do `try`, v `catch` nastav `status.textContent`, zavolej `app.dispose()` a `return`. Přidání třídy `is-loaded` nech až za blokem, aby proběhlo jen po úspěchu.

## --tip--

Stejná kostra u načítání recenzí: v `try` čekáš na `fetch`, v `catch` napíšeš „Recenze se nepodařilo načíst" a skončíš, vykreslení seznamu je až za blokem.

# --explain--

Vysvětli vlastními slovy, proč je náhradní obrázek pod canvasem užitečný i poté, co se scéna jednou načítá bez chyby na tvém počítači.

## --model--

Obrázek je vidět během stahování scény, takže sekce není prázdná. Zůstane i tehdy, když se načtení u návštěvníka pokazí — výpadek sítě, blokovaný soubor, prohlížeč bez WebGL — a na slabých zařízeních, kde scénu schválně nenačítám. U mě to funguje, ale web běží na zařízeních a sítích, které neovlivním.

## --checklist--

- Obrázek zakryje čas, než se scéna stáhne.
- Při chybě načtení zůstane místo prázdné plochy.
- Hodí se i tam, kde scénu schválně nenačítám.
