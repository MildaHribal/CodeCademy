---
title: Podsvícení přes proměnnou scény
see: web-3d-efekty/spline#ovladani-sceny-z-javascriptu
---

# --description--

Podsvícení kláves je ve scéně víc než jedna barva: světlo pod klávesami, záře a jiný odstín pouzdra. Designérka to v editoru navázala na proměnnou `podsviceni` (ano/ne). Kód tedy nepřepíná objekty jeden po druhém, jen nastaví proměnnou a scéna se přepne sama.

Tlačítko `.backlight` je přepínač se stavem v `aria-pressed`.

**Úkol:** Po načtení scény kliknutí na `.backlight` přepne proměnnou scény `podsviceni` na `true`, další kliknutí zpět na `false`. `aria-pressed` tlačítka odpovídá stavu.

# --hints--

Po prvním kliknutí je proměnná `podsviceni` zapnutá a tlačítko stisknuté.
...
const button = document.querySelector('.backlight');
await helpers.click(button);
assert.equal(app.getVariable('podsviceni'), true, 'po kliknutí na Podsvícení má být proměnná scény podsviceni true');
assert.equal(button.getAttribute('aria-pressed'), 'true', 'po zapnutí má mít tlačítko aria-pressed="true"');
```

Druhé kliknutí podsvícení vypne.
...
await helpers.click(button);
await helpers.click(button);
assert.equal(app.getVariable('podsviceni'), false, 'po druhém kliknutí má být proměnná podsviceni false');
assert.equal(button.getAttribute('aria-pressed'), 'false', 'po vypnutí má mít tlačítko aria-pressed="false"');
```
...
Proměnné scény jsou v části [Ovládání scény z JavaScriptu](see:web-3d-efekty/spline#ovladani-sceny-z-javascriptu).

## --tip--

Nový stav spočítej z `aria-pressed` tlačítka, pošli ho do `app.setVariable` se jménem proměnné a zapiš zpátky do `aria-pressed`.

## --tip--

Stejný přepínač u vypínače lampy v prvním workshopu, jen místo intenzity světla nastavuješ proměnnou scény.
