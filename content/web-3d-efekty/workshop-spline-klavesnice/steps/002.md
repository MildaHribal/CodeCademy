---
title: Scéna ze Splinu v canvasu
see: web-3d-efekty/spline#export-a-vlozeni-pres-splinetool-runtime
---

# --description--

Designérka vyexportovala scénu jako `klapka-k75.splinecode`. Ve skutečném projektu by soubor ležel ve složce `public/` vedle webu; v kurzu je export součástí `spline-runtime.js`, takže ho načteš relativní cestou `./klapka-k75.splinecode`.

Scénu spustí dva řádky: `new Application(canvas)` vytvoří přehrávač nad canvasem a `load(cesta)` stáhne a spustí scénu. Import je stejný jako z balíčku, jen z místního souboru.

Na jiných datech, scéna sluchátek:

```js
import { Application } from './spline-runtime.js';

const player = new Application(document.querySelector('.hero__canvas'));
player.load('./sluchatka.splinecode');
```

**Úkol:** V `main.js` naimportuj `Application` ze `./spline-runtime.js`, vytvoř aplikaci nad `canvas` (proměnná už je připravená) a načti scénu `./klapka-k75.splinecode`.

Klávesnice se v náhledu vykreslí přes náhradní obrázek.

# --hints--

Vznikne `Application`, která kreslí do `.explore__canvas`.

```js
const { Application } = await helpers.importFile('spline-runtime.js');
await helpers.importFile('main.js');
const [app] = Application.course.instances;
assert.ok(app, 'main.js má vytvořit new Application(…)');
assert.equal(app.canvas, document.querySelector('.explore__canvas'), 'Application má dostat canvas .explore__canvas');
```

Scéna se načte z místního souboru `./klapka-k75.splinecode`.

```js
const { Application } = await helpers.importFile('spline-runtime.js');
await helpers.importFile('main.js');
const loads = Application.course.log.filter((entry) => entry.method === 'load');
assert.ok(loads.length > 0, 'na aplikaci se má zavolat load(…) s cestou ke scéně');
assert.match(String(loads[0].args[0]), /(^|\/)klapka-k75\.splinecode$/, 'load má dostat cestu ./klapka-k75.splinecode');
const app = Application.course.instances[0];
await helpers.waitFor(() => app.findObjectByName('Klávesnice')).catch(() => {});
assert.ok(app.findObjectByName('Klávesnice'), 'scéna se má načíst — ve scéně má být objekt Klávesnice');
```
...
app.load('./klapka-k75.splinecode');
```
