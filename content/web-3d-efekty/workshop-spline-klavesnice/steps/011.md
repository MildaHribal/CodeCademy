---
title: Tlačítka až po načtení scény
see: web-3d-efekty/spline#typicke-chyby-a-pasti
---

# --description--

Barvy, podsvícení a předvedení fungují až po načtení scény. Předtím tlačítka vypadají normálně, ale nic nedělají — a když se scéna nenačte vůbec, nebudou dělat nic nikdy. Návštěvník by klikal a nevěděl proč.

Tlačítko, které zrovna nejde použít, má být vypnuté: nejde kliknout, nejde na něj přejít klávesnicí a čtečka ohlásí, že je nedostupné.

**Úkol:** Všechna tlačítka `.swatch` a `.toggle` jsou po otevření stránky vypnutá. Po úspěšném načtení scény se zapnou. Když se načtení nepovede, zůstanou vypnutá.

# --hints--

Po otevření stránky jsou všechna tlačítka scény vypnutá.

```js
await helpers.importFile('main.js');
const buttons = [...document.querySelectorAll('.swatch, .toggle')];
assert.equal(buttons.length, 5, 'na stránce má zůstat 5 tlačítek scény');
assert.ok(buttons.every((button) => button.disabled), 'dokud se scéna nenačte, mají být tlačítka .swatch a .toggle vypnutá (disabled)');
```

Po načtení scény jsou tlačítka zapnutá.
...
const buttons = [...document.querySelectorAll('.swatch, .toggle')];
await helpers.waitFor(() => buttons.every((button) => !button.disabled)).catch(() => {});
assert.ok(buttons.every((button) => !button.disabled), 'po načtení scény mají být všechna tlačítka zapnutá');
```

Když se načtení nepovede, tlačítka zůstanou vypnutá.
...
await helpers.waitFor(() => document.querySelector('.explore__status').textContent.trim().length > 0, 3000).catch(() => {});
const buttons = [...document.querySelectorAll('.swatch, .toggle')];
assert.ok(buttons.every((button) => button.disabled), 'po selhání načtení mají tlačítka zůstat vypnutá');
```
...
Past s tlačítky před načtením je v části [Typické chyby a pasti](see:web-3d-efekty/spline#typicke-chyby-a-pasti).

## --tip--

Vypnout je můžeš v HTML atributem, nebo hned na začátku `main.js` projít `document.querySelectorAll('.swatch, .toggle')` a nastavit `disabled`. Zapnutí patří až za místo, kde je jisté, že se scéna načetla.
