---
title: Klik na část klávesnice
see: web-3d-efekty/spline#ovladani-sceny-z-javascriptu
---

# --description--

Designérka dala otočnému knoflíku, mezerníku a klávesám Esc a Enter událost **Mouse Down**. Když na ně návštěvník klikne ve scéně, má se v panelu vedle ukázat, co ta část umí. Texty jsou připravené v objektu `parts` podle jmen objektů ve scéně.

Scéna posílá události přes `app.addEventListener(jméno, handler)`. Handler dostane objekt, ve kterém `target.name` je jméno objektu z editoru. Posluchač se přidává po načtení scény — kandidáti na místo jsou konec `loadScene`, nebo zvláštní funkce, kterou odtamtud zavoláš.

**Úkol:** Po načtení scény poslouchej `mouseDown`. Když má kliknutý objekt text v `parts`, nastav `infoTitle` na jméno objektu a `infoText` na jeho text. Kliknutí na jiný objekt panel nezmění.

# --hints--

Klik na otočný knoflík ve scéně ukáže v panelu jeho jméno a popis.
...
const app = Application.course.instances[0];
...
assert.match(document.querySelector('.explore__info-text').textContent, /Frézovaný hliník/, 'po kliknutí na Otočný knoflík má panel ukázat jeho text z parts');
```

Klik na mezerník přepíše panel na mezerník.
...
assert.ok(app, 'scéna se má načíst');
Application.course.simulate(app, 'mouseDown', 'Otočný knoflík');
Application.course.simulate(app, 'mouseDown', 'Mezerník');
await helpers.flush();
assert.equal(document.querySelector('.explore__info-title').textContent.trim(), 'Mezerník', 'po kliknutí na Mezerník má nadpis panelu být „Mezerník"');
assert.match(document.querySelector('.explore__info-text').textContent, /Stabilizátory/, 'po kliknutí na Mezerník má panel ukázat jeho text');
```

Klik na objekt bez popisu panel nezmění.
...
const app = Application.course.instances[0];
assert.ok(app, 'scéna se má načíst');
Application.course.simulate(app, 'mouseDown', 'Klávesa Enter');
Application.course.simulate(app, 'mouseDown', 'Klávesa Q');
await helpers.flush();
assert.equal(document.querySelector('.explore__info-title').textContent.trim(), 'Klávesa Enter', 'klik na Klávesa Q (nemá popis v parts) nemá panel přepsat');
assert.match(document.querySelector('.explore__info-text').textContent, /v šeru/, 'klik na objekt bez popisu nemá změnit text panelu');
```
...
Události ze scény jsou v části [Ovládání scény z JavaScriptu](see:web-3d-efekty/spline#ovladani-sceny-z-javascriptu).

## --tip--

Za přidáním třídy `is-loaded` zavolej `app.addEventListener('mouseDown', …)`. V handleru si z `parts` vytáhni text podle `event.target.name` a když chybí, skonči.

## --tip--

Stejná úvaha u mapy s pobočkami: `map.on('click', (event) => { const branch = branches[event.feature.name]; if (!branch) return; panel.textContent = branch.address; })`.
