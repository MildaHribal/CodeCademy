---
title: "Opakování: canvas přes celou plochu"
kind: recall
...
Studio Klapka z Brna prodává mechanickou klávesnici K75. Jejich designérka vymodelovala klávesnici ve Splinu a ty ji vložíš do produktové stránky: s náhradním obrázkem, líným načítáním, napojením na tlačítka a scroll a s ohledem na telefony i omezený pohyb.

Kurz běží bez internetu, proto místo balíčku `@splinetool/runtime` používá soubor `spline-runtime.js` se **stejným rozhraním**. Místo WebGL kreslí klávesnici do 2D canvasu, ale metody a události jsou stejné jako u skutečného runtime. Ve vlastním projektu jen vyměníš import. Tenhle soubor neupravuj.

V sekci „Prohlédni si ji zblízka" je `.explore__stage`: plocha s náhradním obrázkem (SVG klávesnice) a canvasem, do kterého se bude kreslit scéna. Canvas je zatím malý obdélník 300 × 150 pod obrázkem.

**Úkol:** V `styles.css` nastav `.explore__canvas` tak, aby ležel přes celou plochu `.explore__stage`, přesně nad náhradním obrázkem.

# --hints--

Canvas `.explore__canvas` zabírá celou plochu `.explore__stage`.

```js
const stage = document.querySelector('.explore__stage').getBoundingClientRect();
const canvas = document.querySelector('.explore__canvas').getBoundingClientRect();
const same = ['top', 'left', 'width', 'height'].every((side) => Math.abs(stage[side] - canvas[side]) < 1.5);
assert.ok(same, `canvas má zabírat celou plochu .explore__stage (${Math.round(stage.width)} × ${Math.round(stage.height)}), zabírá ${Math.round(canvas.width)} × ${Math.round(canvas.height)}`);
```

Canvas leží přes náhradní obrázek, ne pod ním.
...
assert.equal(hit, document.querySelector('.explore__canvas'), 'uprostřed plochy má být navrchu canvas, ne náhradní obrázek');
```
...
Roztažení přes rodiče znáš z [lekce o pozicování](see:css-pozicovani/position#inset-roztazeni-a-velikost). Náhradní obrázek `.explore__poster` je napsaný stejně.

## --tip--

Podívej se do `styles.css`, jak je nastavený `.explore__poster`, a napiš stejné pravidlo pro canvas. Canvas je v HTML za obrázkem, takže při stejném pozicování skončí navrchu.
