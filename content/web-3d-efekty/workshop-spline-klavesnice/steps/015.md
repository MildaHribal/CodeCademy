---
title: Kolegovo „zjednodušení" observeru
kind: debug
...
Kolega Ondřej při úklidu kódu smazal v callbacku `IntersectionObserver` jeden řádek, protože mu „přišel zbytečný — callback se přece volá, až když je sekce vidět". Od té doby Lighthouse u stránky hlásí, že se hned po otevření stahuje scéna ze Splinu, přestože je sekce s klávesnicí hluboko pod úvodem. Náhradní obrázek je nahoře navíc schovaný dřív, než k němu kdokoli doscrolluje.

## Úkol

Vrať línému načítání funkčnost: scéna se nesmí načíst dřív, než sekce vjede do okna.

## Co ses naučil

Vložil jsi scénu ze Splinu tak, jak se to dělá na produkčním webu: z vlastního souboru, líně, s náhradním obrázkem a obsluhou chyby, s tlačítky, která ovládají scénu přes jména objektů, proměnné a události, s natáčením při scrollu a s ohledem na telefony i omezený pohyb. Stejné zásady platí pro každé těžké médium — video, mapu nebo 3D model z Three.js.

V labu to samé postavíš v Reactu s komponentou `@splinetool/react-spline`.

## Udělej po svém

- **Vzhled:** když se scéna nenačte nebo je obrazovka úzká, přepínej barvu kloboučků aspoň na náhradním obrázku (SVG má barvu v atributu `fill`).
- **Chování:** přidej natočení za myší — poloha kurzoru od −1 do 1 přes celou sekci do `rotation.x` klávesnice, s plynulým dojezdem.
- **Výkon:** načítej scénu s rezervou 200 px (`rootMargin`), ať je hotová dřív, než k ní návštěvník dojede.
- **Tvoje téma:** vymodeluj si ve Splinu vlastní produkt, vyexportuj ho a vlož do svého projektu s tímhle kódem.

# --hints--

Po otevření stránky, dokud sekce není v okně, se scéna nenačítá.

```js
const { Application } = await helpers.importFile('spline-runtime.js');
await helpers.importFile('main.js');
await helpers.wait(300);
assert.equal(Application.course.instances.length, 0, `dokud sekce se scénou není v okně, nemá vzniknout žádná Application, vzniklo ${Application.course.instances.length}`);
assert.ok(!document.querySelector('.explore__stage').classList.contains('is-loaded'), 'dokud sekce není v okně, má náhradní obrázek zůstat');
```

Po vjezdu sekce do okna se scéna načte právě jednou.
...
stage.scrollIntoView();
await helpers.wait(300);
assert.ok(stage.classList.contains('is-loaded'), 'po vjezdu sekce do okna se má scéna načíst');
assert.equal(Application.course.instances.length, 1, 'scéna se má načíst právě jednou');
```
...
Past, kterou Ondřej zavlekl, je popsaná v části [Typické chyby a pasti](see:web-3d-efekty/spline#typicke-chyby-a-pasti) a ukázaná v předpovědi v části o výkonu.

## --tip--

Callback se nevolá jen při vjezdu. Kdy se zavolá poprvé a s jakou hodnotou `isIntersecting`?

## --tip--

Stejná oprava u videa, které se má přehrát až na obrazovce: v callbacku nejdřív podmínka, která při `isIntersecting` rovném `false` video pozastaví a skončí, a teprve za ní `video.play()`.
