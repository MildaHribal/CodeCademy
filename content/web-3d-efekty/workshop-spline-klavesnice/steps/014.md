---
title: Omezený pohyb
see: web-3d-efekty/spline#pristupnost-a-kdy-spline-nepouzit
---

# --description--

Scéna klávesnice má naklikanou jemnou animaci (klávesnice se lehce vznáší) a stránka ji natáčí při scrollu. Návštěvníkovi, který má v systému zapnuté omezení pohybu, to může dělat zle.

S omezeným pohybem má scéna zůstat vidět a klikací, jen se sama nehýbe: přehrávání scény se po načtení zastaví a natočení při scrollu se vůbec nepřipojí. Preferenci stačí přečíst ve chvíli načítání scény.

**Úkol:** Když má návštěvník zapnutý omezený pohyb, po načtení scény zastav její přehrávání a nepřipojuj natáčení při scrollu. Bez omezeného pohybu funguje všechno jako dosud.

# --hints--

S omezeným pohybem se přehrávání scény po načtení zastaví.
...
try {
  ...
  assert.ok(stage.classList.contains('is-loaded'), 'scéna se má s omezeným pohybem dál načíst');
  assert.ok(Application.course.log.some((entry) => entry.method === 'stop'), 's omezeným pohybem se má po načtení zavolat app.stop()');
} finally {
  Object.defineProperty(MediaQueryList.prototype, 'matches', matches);
}
```

S omezeným pohybem se klávesnice při scrollu nenatáčí.
...
try {
  ...
  stage.scrollIntoView();
  await helpers.waitFor(() => stage.classList.contains('is-loaded'), 3000).catch(() => {});
  const keyboard = Application.course.instances[0]?.findObjectByName('Klávesnice');
  assert.ok(keyboard, 'scéna se má načíst');
  const before = keyboard.rotation.y;
  window.scrollTo(0, 0);
  await helpers.wait(300);
  assert.equal(keyboard.rotation.y, before, 's omezeným pohybem se klávesnice při scrollu nemá natáčet');
} finally {
  Object.defineProperty(MediaQueryList.prototype, 'matches', matches);
}
```

Bez omezeného pohybu se scéna nezastaví a při scrollu se natáčí.
...
assert.ok(keyboard, 'scéna se má načíst');
window.scrollTo(0, 0);
await helpers.waitFor(() => Math.abs(keyboard.rotation.y + 0.6) < 0.01).catch(() => {});
assert.ok(Math.abs(keyboard.rotation.y + 0.6) < 0.01, 'bez omezeného pohybu se má klávesnice při scrollu natáčet');
...
Co s omezeným pohybem udělat ve scéně ze Splinu, je v části [Přístupnost a kdy Spline nepoužít](see:web-3d-efekty/spline#pristupnost-a-kdy-spline-nepouzit).

## --tip--

Po přidání `is-loaded` zjisti preferenci přes `window.matchMedia` a její `matches`. Podle ní zavolej `stop` na aplikaci a připojení posluchače scrollu obal podmínkou.

## --tip--

Stejné rozhodnutí u automatického přehrávání videa na pozadí: když návštěvník chce omezený pohyb, `video.pause()` a posluchač, který video posouvá při scrollu, se nepřipojí.

# --explain--

Vysvětli vlastními slovy, proč u scény ze Splinu s omezeným pohybem nestačí zastavit přehrávání přes `app.stop()`.

## --model--

`app.stop()` zastaví jen animace, které běží uvnitř scény, třeba vznášení naklikané v editoru. Natáčení při scrollu je ale můj kód na stránce, který objekt scény mění přímo, a ten by po zastavení scény klávesnici dál natáčel. Omezený pohyb proto musím respektovat na obou místech: ve scéně i v kódu, který scénu řídí.

## --checklist--

- `app.stop()` zastaví animace uvnitř scény.
- Natáčení při scrollu je kód stránky, který běží dál.
- Pohyb se vypíná ve scéně i v kódu, který ji řídí.
