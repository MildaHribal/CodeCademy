---
title: Kód zkopírovaný z exportu
kind: debug
...
Kolegyně Petra vyměnila cestu ke scéně za tu, kterou nabízí Spline v okně **Export → Code**, „ať máme vždycky nejnovější verzi od designérky". U ní doma to fungovalo. Na veletrhu, kde stánek jede na offline notebooku, ale zůstane vidět jen náhradní obrázek a konzole hlásí:
...
Oprav načítání tak, aby scéna nezávisela na serveru Splinu a fungovala i bez internetu. Soubor exportu je u webu jako `./klapka-k75.splinecode`.

# --hints--

Scéna se načte a náhradní obrázek zmizí i bez internetu.
...
assert.ok(stage.classList.contains('is-loaded'), 'scéna se má načíst a .explore__stage dostat třídu is-loaded');
assert.ok(Application.course.instances[0]?.findObjectByName('Klávesnice'), 've scéně má být objekt Klávesnice');
```

Cesta ke scéně nevede na cizí server.

```js
const { Application } = await helpers.importFile('spline-runtime.js');
await helpers.importFile('main.js');
const load = Application.course.log.find((entry) => entry.method === 'load');
assert.ok(load, 'na aplikaci se má zavolat load(…)');
assert.doesNotMatch(String(load.args[0]), /^(https?:)?\/\//, `scéna se má načítat z místního souboru, ne z ${load.args[0]}`);
```
...
Hláška a příčina jsou v pasti v části [Export a vložení přes @splinetool/runtime](see:web-3d-efekty/spline#export-a-vlozeni-pres-splinetool-runtime).

## --tip--

Podívej se, jakou adresu dostává `app.load`. Na veletrhu není internet, ale soubor exportu leží přímo u stránky.

## --tip--

Stejná oprava u fontu: místo odkazu na cizí server `https://fonts.example.com/inter.woff2` načteš soubor uložený u webu, `./fonts/inter.woff2`.
...
async function loadScene() {
  await app.load('https://prod.spline.design/kPq3xR7Tn2Lw9sVb/scene.splinecode');
