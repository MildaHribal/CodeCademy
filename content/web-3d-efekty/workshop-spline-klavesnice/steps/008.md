---
title: Barva kloboučků z tlačítek
see: web-3d-efekty/spline#ovladani-sceny-z-javascriptu
---

# --description--

Zákazník si vybírá barvu kloboučků kláves. Tlačítka vedle scény mají barvu v `data-keycaps` a stav v `aria-pressed`. Ve scéně je všechny kloboučky nese objekt „Kloboučky kláves" — jméno, na kterém ses s designérkou domluvil.

Objekt ze scény najdeš podle jména a jeho barvu změníš zápisem CSS barvy do vlastnosti. Hledat ho má smysl až po načtení.

**Úkol:** Po načtení scény po kliknutí na kterékoli tlačítko barvy nastav objektu „Kloboučky kláves" barvu z `data-keycaps` a kliknutému tlačítku `aria-pressed="true"`, ostatním `"false"`.

# --hints--

Po kliknutí na „Grafitová" mají kloboučky ve scéně barvu `#334155`.
...
await helpers.click(document.querySelector('[data-keycaps="#334155"]'));
assert.equal(app.findObjectByName('Kloboučky kláves').color, '#334155', 'po kliknutí na Grafitová má mít objekt Kloboučky kláves barvu #334155');
```

Každé tlačítko nastaví svou barvu a stisknuté je jen poslední.
...
const app = Application.course.instances[0];
assert.ok(app, 'scéna se má načíst');
await helpers.click(document.querySelector('[data-keycaps="#99f6e4"]'));
assert.equal(app.findObjectByName('Kloboučky kláves').color, '#99f6e4', 'po kliknutí na Mátová má mít objekt Kloboučky kláves barvu #99f6e4');
const pressed = [...document.querySelectorAll('.swatch')].map((swatch) => swatch.getAttribute('aria-pressed'));
assert.deepEqual(pressed, ['false', 'false', 'true'], 'po kliknutí na Mátová má mít aria-pressed="true" jen ono');
await helpers.click(document.querySelector('[data-keycaps="#f1f5f9"]'));
assert.equal(app.findObjectByName('Kloboučky kláves').color, '#f1f5f9', 'po kliknutí zpět na Mléčná má mít objekt barvu #f1f5f9');
```
...
Hledání objektu podle jména a jeho barva jsou v části [Ovládání scény z JavaScriptu](see:web-3d-efekty/spline#ovladani-sceny-z-javascriptu), i s pastí `Cannot set properties of undefined`.

## --tip--

Po načtení si ulož `app.findObjectByName('Kloboučky kláves')`. Projdi `.swatch` a v obsluze kliknutí nastav objektu `color` a přepiš `aria-pressed` všem tlačítkům.

## --tip--

Stejný vzor jako u barvy stínidla v konfigurátoru lampy, jen místo materiálu Three.js měníš vlastnost objektu ze Splinu.
