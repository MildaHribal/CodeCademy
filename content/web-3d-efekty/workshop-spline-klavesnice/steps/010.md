---
title: Předvedení stisku z tlačítka
see: web-3d-efekty/spline#ovladani-sceny-z-javascriptu
---

# --description--

Stisk mezerníku má ve scéně naklikanou animaci na událost **Mouse Down**. Myší si ji návštěvník spustí kliknutím na mezerník. Kdo ale ovládá stránku klávesnicí nebo čtečkou, na objekt ve 3D nedosáhne. Proto je vedle tlačítko „Předvést stisk mezerníku", které spustí tutéž animaci z kódu.

Kandidáti: `emitEvent` na aplikaci s názvem události a jménem objektu, nebo stejnojmenná metoda přímo na objektu nalezeném podle jména.

**Úkol:** Po načtení scény kliknutí na tlačítko `.demo` spustí na objektu „Mezerník" událost `mouseDown`.

# --hints--

Kliknutí na „Předvést stisk mezerníku" spustí ve scéně `mouseDown` na objektu Mezerník.
...
await helpers.click(document.querySelector('.demo'));
const emitted = Application.course.log.filter((entry) => entry.app === app && entry.method === 'emitEvent');
assert.ok(emitted.some((entry) => entry.args[0] === 'mouseDown' && entry.args[1] === 'Mezerník'), 'po kliknutí na tlačítko se má na scéně spustit emitEvent("mouseDown", "Mezerník")');
```

Tlačítko spustí událost při každém kliknutí.
...
await helpers.click(button);
const emitted = Application.course.log.filter((entry) => entry.app === app && entry.method === 'emitEvent' && entry.args[1] === 'Mezerník');
assert.equal(emitted.length, 2, `dvě kliknutí mají spustit událost dvakrát, spustila ${emitted.length}`);
```
...
Spuštění události z kódu a proč ho HTML tlačítko potřebuje, je v části [Ovládání scény z JavaScriptu](see:web-3d-efekty/spline#ovladani-sceny-z-javascriptu).

## --tip--

Posluchač `click` na `.demo` přidej po načtení a zavolej v něm `emitEvent` s názvem události a jménem objektu přesně podle editoru.

## --tip--

Stejně by tlačítko „Otevřít víčko" spustilo animaci lahve: `app.emitEvent('mouseDown', 'Víčko lahve')`.
