---
title: Líné načítání scény
see: web-3d-efekty/spline#vykon-velikost-sceny-line-nacitani-a-nahradni-obrazek
---

# --description--

Scéna je až ve druhé sekci stránky, pod úvodem. Přesto se teď stahuje a spouští hned po otevření — i lidem, kteří si přečtou nadpis a odejdou. Načti ji až ve chvíli, kdy sekce vjede do okna.

Na to je `IntersectionObserver`. Pozor na dvě věci z lekce: callback se zavolá hned po `observe` i pro prvek mimo okno (s `isIntersecting: false`), a při každém dalším vjezdu znovu. Proto kontrola `isIntersecting` na začátku a `observer.disconnect()`, jakmile načítání spustíš.

Vytvoření aplikace se přesune dovnitř `loadScene` — aplikace má vzniknout až při načítání, ne při otevření stránky.

Na jiných datech, mapa, která se načte až u patičky:
...
mapObserver.observe(document.querySelector('.contact__map'));
```

**Úkol:**

1. Přesuň `new Application(canvas)` na začátek funkce `loadScene`.
2. Místo přímého zavolání `loadScene()` vytvoř `IntersectionObserver`, který sleduje `stage` a při jeho vjezdu do okna se odpojí a zavolá `loadScene()`.

# --hints--

Po otevření stránky, dokud sekce není v okně, žádná `Application` nevznikne.
...
Když sekce vjede do okna, scéna se načte a obrázek zmizí.
...
assert.equal(Application.course.instances.length, 1, 'po vjezdu sekce do okna má vzniknout jedna Application');
assert.ok(stage.classList.contains('is-loaded'), 'po vjezdu sekce do okna se má scéna načíst a .explore__stage dostat is-loaded');
```

Opakovaný vjezd sekce do okna nevytvoří další aplikaci.
...
const stage = document.querySelector('.explore__stage');
stage.scrollIntoView();
await helpers.waitFor(() => Application.course.instances.length > 0, 3000).catch(() => {});
window.scrollTo(0, 0);
await helpers.wait(300);
stage.scrollIntoView();
await helpers.wait(400);
assert.equal(Application.course.instances.length, 1, `po odjetí a novém vjezdu sekce má pořád existovat jen jedna Application, je jich ${Application.course.instances.length} — odpoj observer`);
```

# --explain--

Vysvětli vlastními slovy, proč scénu ze Splinu nenačítáš hned při otevření stránky a proč callback observeru kontroluje `isIntersecting`.

## --model--

Scéna je těžká: runtime, soubor scény a práce procesoru stojí data i baterii, a spousta návštěvníků do druhé sekce vůbec nedojede. Proto ji načítám, až se sekce objeví v okně. Callback observeru se ale zavolá hned po začátku sledování i pro prvek mimo okno, s `isIntersecting` nastaveným na `false`, takže bez kontroly by se scéna načetla okamžitě. Po spuštění načítání observer odpojím, aby se scéna nenačítala při každém dalším vjezdu.

## --checklist--

- Scéna je těžká a část návštěvníků k ní nedojede.
- Načítá se, až když se sekce objeví v okně.
- Callback přichází hned po `observe` i pro prvek mimo okno.
- Odpojení observeru zabrání opakovanému načtení.
